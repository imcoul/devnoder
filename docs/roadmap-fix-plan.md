...
**Phases 0–6, 9, and 11 are done in `devnoder` on `claude/devnoder-wrangler-issues-kyhb0i` (PR #2), and Phase 8 is done in the separate `devnoder-cli` repo on `claude/devnoder-handover-docs-834g2y` (commit `07d51c1`, pushed, no PR opened yet) — all verified against real running instances or a real green CI-equivalent local run, not just typechecked.** `devnoder` commits, in order: `68ba368` (Phase 0), `4e01f2a` (Phase 1), `ef93fbf` (Phase 2), `40fbd50` (Phase 3), `50a47fd` (Phase 4), `7a4a2a8` (skill fix), `2eb6256` (Phase 5), plus follow-up Phase 6, Phase 9, and Phase 11 commits. Phase 7 remains blocked (needs the project owner's Cloudflare secrets, and no source for its Workers exists in any repo in scope). Phase 10 depends on Phase 9's identity primitive getting a real counterparty to verify against — Stage 1 (a LogoForge preset entry, no live signing) is unblocked and is the only fully open item besides Phase 7.
...
## Phase 11 — Tests, CI, and honest docs (P1/P2) ✅ Done (this repo's test/lint/CI setup — `devnoder-cli` and the executor Worker's routes remain untested), verified

- **`vitest` is wired up** (`vitest.config.ts`, `src/test/setupTests.ts` polyfilling `indexedDB` via `fake-indexeddb` — no jsdom needed, WebCrypto is native in Node 20+). 19 real tests across the three Phase 9 modules: `ConnectionSigning.test.ts` (keypair generation, sign/verify round-trip, tamper detection, cross-identity rejection, revoke), `MCPConfigStore.test.ts` (capability persistence, header encryption round-trip, audit log ordering/survival-after-delete, the "never log values" schema guarantee), `MCPClient.test.ts` (the capability gate actually rejects synchronously, without attempting a transport connection; a call to a never-connected server is denied and audit-logged). Chose these over `GitService.ts`/`ProjectService.ts` (the roadmap's original suggestion) because they're the most recently-written, most security-relevant code with the clearest pass/fail contracts — `GitService`/`ProjectService` remain untested and are a good next target.
- **`npm run lint` was completely broken** — no `eslint.config.js` existed (ESLint v9+ requires flat config) and `eslint` wasn't even a declared dependency. Fixed: added `eslint`/`typescript-eslint`/`eslint-plugin-react-hooks` + `eslint.config.js`. First real lint run surfaced 74 errors/149 warnings across ~15 pre-existing files this phase doesn't otherwise touch (regex-escape issues, including inside `SecretDetector.ts` — security-sensitive matching logic a blind autofix could silently change) plus `eslint-plugin-react-hooks` v7's new React-Compiler-oriented `recommended` rules firing across most of the codebase's existing effects. Deliberately did not mass-fix ~15 unrelated files as a side effect of turning on a linter for the first time — downgraded those rule categories to warnings (visible in `npm run lint` output, non-blocking) with the reasoning written directly into `eslint.config.js`, and kept only `react-hooks/rules-of-hooks` (error) + `exhaustive-deps` (warn) instead of the full v7 `recommended`. `npm run lint` now exits 0.
- **CI now runs `typecheck → lint → test → build`** in `.github/workflows/ci.yml`, in that order, on every push/PR to `main` — verified by running the exact sequence locally end-to-end (all four green) before committing, not just added and hoped.
- **Not done here**: `devnoder-cli` has no test suite yet; the executor Worker's real routes / `501` contract can't be tested since (per Phase 7) no source for it exists in any repo in scope.
